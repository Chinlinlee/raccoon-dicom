...
function dicomFileToFHIRImagingStudy(filename) {
    try {
        ...
        let ANandIssuer = getDicomParserTagStringConcat(
            dataset,
            "x00080050",
            "x00080051"
        );
        ...
        if (patientId) {
            studyobj.subject.reference =
                "Patient/" + dataset.string("x00100020");
        ...
        let imaging_started =
            dataset.string("x00080020") + dataset.string("x00080030");
        ...
        let series_started =
            dataset.string("x00080021") + dataset.string("x00080031");
        const series_date = moment(series_started, "YYYYMMDDhhmmss").toDate();
        study_series_obj.started =
            series_date != null ? series_date : undefined;
        study_series_obj.performer =
            dataset.string("x00081050") ||
            dataset.string("x00081052") ||
            dataset.string("x00081070") ||
            dataset.string("x00081072");
        ...
        study_series_insatance_obj.sopClass.code =
            "urn:oid:" + dataset.string("x00080016");
        study_series_insatance_obj.number = dataset.intString("x00200013");
        study_series_insatance_obj.title =
            dataset.string("x00080008") ||
            dataset.string("x00070080") ||
            (dataset.string("x0040a043") != undefined
                ? dataset.string("x0040a043") + dataset.string("x00080104")
                : undefined) ||
            dataset.string("x00420010");
        let imagingStudyJson = combineImagingStudyClass(
            studyobj,
            study_series_obj,
            study_series_insatance_obj
        );
...
function dicomJsonToFHIRImagingStudy(dicomJson) {
    ...
    if (patientId) {
        studyObj.subject.reference =
            "Patient/" + dcm2jsonV8.dcmString(dicomJson, "00100020");
        studyObj.subject.type = "Patient";
        studyObj.subject.identifier.use = "usual";
        studyObj.subject.identifier.value = dcm2jsonV8.dcmString(
            dicomJson,
            "00100020"
        );
    } else {
    ...
    let seriesStarted =
        dcm2jsonV8.dcmString(dicomJson, "00080021") +
        dcm2jsonV8.dcmString(dicomJson, "00080031");
    seriesObj.started = moment(seriesStarted, "YYYYMMDDhhmmss");
    seriesObj.started = seriesObj.started.isValid()
        ? seriesObj.started.toISOString()
        : undefined;
    seriesObj.performer =
        dcm2jsonV8.dcmString(dicomJson, "00081050") ||
    ...
    instanceObj.sopClass.code =
        "urn:oid:" + dcm2jsonV8.dcmString(dicomJson, "00080016");
    instanceObj.number = dcm2jsonV8.dcmString(dicomJson, "00200013");
    instanceObj.title =
        dcm2jsonV8.dcmString(dicomJson, "00080008") ||
        dcm2jsonV8.dcmString(dicomJson, "00070080") ||
        (dcm2jsonV8.dcmString(dicomJson, "0040a043") != undefined
            ? dcm2jsonV8.dcmString(dicomJson, "0040a043") +
              dcm2jsonV8.dcmString(dicomJson, "00080104")
            : undefined) ||
        dcm2jsonV8.dcmString(dicomJson, "00420010");
    //#endregion
...
function combineImagingStudyClass(
    imagingStudy,
    imagingStudySeries,
